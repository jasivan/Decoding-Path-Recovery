...
#     """Extracts step-0 layer metrics, isolating the separate raw probabilities 
#        for the Top-1 and Top-2 predicted tokens per layer from JSON traces.
#     """
#     records = []
...
#                 # Token-level correctness grouping condition
...
#                         # Explicitly extract distinct target probabilities for Top-1 and Top-2
...
#                         # Calculate raw sequence difference right away
#                         prob_diff = abs(p1 - p2)
...
# def plot_layer_margin_grid(df, model_name="Llama", num_last_layers=15):
#     """Plots the mean probability difference between the Top-1 and Top-2 tokens.
#        Scales the vertical log viewport dynamically based on data limits per model.
#     """
#     model_df = df[df["Model"] == model_name].copy()
#     if model_df.empty:
#         print(f"No metric traces found for model: {model_name}")
#         return

#     # Drop layer 0 to avoid scale distortion
#     model_df = model_df[model_df["Layer"] > 0].copy()
...
#     # --- DYNAMIC Y-LIMIT DETECTOR PER MODEL ---
#     formats = ["Symbolic_Equation", "Worded_Question"]
#     observed_min_diff = 1.0
    
#     for fmt in formats:
#         slice_df = model_df[model_df["Format"] == fmt]
#         if not slice_df.empty:
#             # Aggregate to locate the lowest mean difference plotted
#             sum_check = slice_df.groupby(["Layer", "Status"]).mean(numeric_only=True).reset_index()
#             valid_vals = sum_check["Prob_Diff"][sum_check["Prob_Diff"] > 0]
#             if not valid_vals.empty:
#                 observed_min_diff = min(observed_min_diff, valid_vals.min())
    
#     # Set custom bounds for high-confidence runs (like Qwen) versus deeply chaotic runs
#     if observed_min_diff >= 0.09:
#         dynamic_floor = 0.1
#     else:
#         dynamic_floor = 10 ** np.floor(np.log10(observed_min_diff))
#         dynamic_floor = max(dynamic_floor, 1e-6)  # Safety visual bound
#     # ------------------------------------------
...
#     # Primary theme colors for the status groups
...
#         if slice_df.empty:
#             ax_margin.text(0.5, 0.5, "No Format Vector Data", ha='center', va='center')
#             continue
            
#         # 1. Compute uncorrupted pure layer averages (Mean)
#         summary = slice_df.groupby(["Layer", "Status"]).mean(numeric_only=True).reset_index()
#         accuracy_summary = slice_df.groupby("Layer")["Layer_Correct"].mean().reset_index()
        
#         # Ensure structural integrity across missing categorical rows
...
#         # Apply the visual threshold clamp to the calculated mean differences
#         summary["Prob_Diff"] = summary["Prob_Diff"].clip(lower=dynamic_floor)
        
#         # 2. Linear background execution bars
#         ax_bars = ax_margin.twinx()
#         ax_bars.bar(
#             accuracy_summary["Layer"],
#             accuracy_summary["Layer_Correct"], 
#             color="#e3e3e3", width=0.55, edgecolor="#cccccc", alpha=0.4, zorder=1
#         )
#         ax_bars.set_ylim(-0.05, 1.05)
#         ax_bars.get_yaxis().set_visible(False)
        
#         # 3. Plot Logarithmic Probability Mean Difference Curves
...
#         ax_margin.set_xlabel("Transformer Layer Index (Depth)", fontweight='bold', fontsize=12)
#         ax_margin.set_ylabel("Mean Probability Difference (P_Top1 - P_Top2) [Log Scale]", fontsize=12, color='#333333')
#         ax_margin.set_xticks(all_layers)
#         ax_margin.set_title(format_titles[idx], fontsize=15, fontweight='bold', pad=14)
#         ax_margin.grid(True, linestyle=':', alpha=0.5, color='#999999', zorder=0)

#     # ---- Outside Legend Construction ----
#     legend_elements = [
#         plt.Rectangle((0,0), 1, 1, color="#e3e3e3", ec="#cccccc", alpha=0.5, label="Layer Accuracy (Linear Scale)"),
#         Line2D([0], [0], color=colors["Correct"], lw=3, linestyle="-", marker="o", label="Correct Sequence: Mean Margin"),
#         Line2D([0], [0], color=colors["Incorrect"], lw=3, linestyle="-", marker="o", label="Incorrect Sequence: Mean Margin")
#     ]
    
#     fig.legend(
#         handles=legend_elements, loc='center left', bbox_to_anchor=(0.93, 0.5),
#         frameon=True, facecolor='white', edgecolor='#cccccc',
#         title="Prediction Competition Layers", title_fontsize=12, fontsize=11
#     )
    
#     plt.suptitle(
#         f"{model_name}: Mean Probability Difference Between Top Two Placed Tokens\n"
#         f"Wider Gaps Signify High Decision Commitment | Viewport Floor: {dynamic_floor}", 
#         y=0.98, fontsize=16, fontweight='bold'
#     )
    
#     out_path = os.path.join(OUTPUT_DIR, f"{model_name}_mean_probability_differences.png")
#     plt.savefig(out_path, dpi=300, bbox_inches='tight')
#     plt.close()
#     print(f" -> Mean difference matrix exported to: {out_path}")
...
#         unique_models = margin_df["Model"].unique()
#         print(f"Located metrics for models: {list(unique_models)}")
        
#         for model in unique_models:
#             print(f"Processing evaluation matrix generation for: {model}")
#             plot_layer_margin_grid(margin_df, model_name=model, num_last_layers=15)
#         print("Success! All model evaluation matrices exported cleanly.")
#     else:
...
#                         # 2. Chosen (Top-1) vs Gold Token Probability Margin
#                         p_gold = float(gold_tracking.get("prob", 0.0))
#                         # For correct samples, Chosen == Gold, so the difference is trivially 0
...
# def plot_layer_margin_grid(df, model_name="Llama", num_last_layers=15):
#     """Plots the mean probability difference between Top-1 and Top-2 tokens, 
#        overlaying a black reference curve tracking the explicit gap between 
#        the chosen mistake and the true answer token.
#     """
#     model_df = df[df["Model"] == model_name].copy()
#     if model_df.empty:
#         print(f"No metric traces found for model: {model_name}")
...
#         if not slice_df.empty:
#             sum_check = slice_df.groupby(["Layer", "Status"]).mean(numeric_only=True).reset_index()
#             # Scan both difference vectors to set the absolute lowest non-zero viewport floor
...
#         # Apply the log-scale visual clamp across both columns
#         summary["Prob_Diff"] = summary["Prob_Diff"].clip(lower=dynamic_floor)
...
#                 marker="o", markersize=7, zorder=3  # <-- CHANGED HERE FROM shortcuts_size=7
#             )
        
#         # 4. --- OVERLAY SPECIFIC CHOSEN VS GOLD DISCREPANCY (ONLY INCORRECT) ---
...
#         ax_margin.set_ylim(bottom=dynamic_floor, top=1.05)
#         ax_margin.set_xlabel("Transformer Layer Index (Depth)", fontweight='bold', fontsize=12)
#         ax_margin.set_ylabel("Mean Probability Margin [Log Scale]", fontsize=12, color='#333333')
#         ax_margin.set_xticks(all_layers)
#         ax_margin.set_title(format_titles[idx], fontsize=15, fontweight='bold', pad=14)
#         ax_margin.grid(True, linestyle=':', alpha=0.5, color='#999999', zorder=0)
...
#     plt.suptitle(
#         f"{model_name}: Layer Margin Profiles & Target Token Alignment\n"
#         f"Black Curve Quantifies Mistake Severity (Distance to Gold Answer) | Viewport Floor: {dynamic_floor}", 
#         y=0.98, fontsize=16, fontweight='bold'
#     )
...
#     plt.savefig(out_path, dpi=300, bbox_inches='tight')
#     plt.close()
#     print(f" -> Enhanced difference matrix exported to: {out_path}")
