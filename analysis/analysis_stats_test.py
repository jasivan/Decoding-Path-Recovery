...
# def run_granular_tables_with_emojis(matrix_csv="fermat_matrix.csv"):
#     if not os.path.exists(matrix_csv):
#         print(f"❌ Error: Could not find '{matrix_csv}'.")
#         return

#     df = pd.read_csv(matrix_csv)
#     all_columns = df.columns.tolist()
#     models = sorted(list(set(col.split(" (")[0] for col in all_columns if " (" in col)))

#     ALPHA = 0.05

#     # Define slices
#     slice_types = {
#         "Subgroup": df["Subgroup"].dropna().unique().tolist(),
#         "Operation": [op for op in ["addition", "subtraction", "multiplication", "division"] if op in df["Operation"].unique()]
#     }

#     for model in models:
#         for track_type in ["Q", "Eq"]:
#             base_col = f"{model} (Base {track_type})"
#             ml_col = f"{model} (ML {track_type})"
#             tk_col = f"{model} (TK {track_type})"

#             if not (base_col in df and ml_col in df and tk_col in df):
#                 continue

#             print("\n" + "="*90)
#             print(f"🧠 MODEL TRACK EVALUATION: {model} [{track_type}]")
#             print("="*90)

#             for slice_name, slice_values in slice_types.items():
#                 print(f"\n📊 Slicing Matrix by {slice_name.upper()}:")
                
#                 # Table Header Formatting
#                 header = f"{'Slice Value':<16} | {'N':<5} | {'Base vs ML (b/c)':<17} | {'ML p-val':<10} | {'ML H0':<6} | {'Base vs TK (b/c)':<17} | {'TK p-val':<10} | {'TK H0':<6}"
#                 print("-" * len(header))
#                 print(header)
#                 print("-" * len(header))

#                 for val in slice_values:
#                     sliced_df = df[df[slice_name] == val].copy()
                    
#                     clean_df = sliced_df[
#                         (sliced_df[base_col].isin([0, 1, 0.0, 1.0])) & 
#                         (sliced_df[ml_col].isin([0, 1, 0.0, 1.0])) & 
#                         (sliced_df[tk_col].isin([0, 1, 0.0, 1.0]))
#                     ]

#                     if len(clean_df) < 2:
#                         continue

#                     y_base = clean_df[base_col].astype(int).values
#                     y_ml = clean_df[ml_col].astype(int).values
#                     y_tk = clean_df[tk_col].astype(int).values

#                     # Base vs ML
#                     t_ml = pd.crosstab(y_base, y_ml)
#                     for i in [0, 1]:
#                         if i not in t_ml.index: t_ml.loc[i] = 0
#                         if i not in t_ml.columns: t_ml[i] = 0
#                     t_ml = t_ml.sort_index(axis=0).sort_index(axis=1).values
#                     b_ml, c_ml = t_ml[1, 0], t_ml[0, 1]
#                     p_ml = mcnemar(t_ml, exact=True).pvalue
#                     p_ml_one = p_ml / 2 if c_ml >= b_ml else 1.0 - (p_ml / 2)
#                     # ✅ Reject H0 (Significant Improvement) vs ❌ Accept H0 (Noise)
#                     h0_ml_status = "✅ REJ" if p_ml_one < ALPHA else "❌ ACC"

#                     # Base vs TK
#                     t_tk = pd.crosstab(y_base, y_tk)
#                     for i in [0, 1]:
#                         if i not in t_tk.index: t_tk.loc[i] = 0
#                         if i not in t_tk.columns: t_tk[i] = 0
#                     t_tk = t_tk.sort_index(axis=0).sort_index(axis=1).values
#                     b_tk, c_tk = t_tk[1, 0], t_tk[0, 1]
#                     p_tk = mcnemar(t_tk, exact=True).pvalue
#                     p_tk_one = p_tk / 2 if c_tk >= b_tk else 1.0 - (p_tk / 2)
#                     h0_tk_status = "✅ REJ" if p_tk_one < ALPHA else "❌ ACC"

#                     # Print formatted table row
#                     ml_dis = f"{b_ml}/{c_ml}"
#                     tk_dis = f"{b_tk}/{c_tk}"
#                     print(f"{val:<16} | {len(clean_df):<5} | {ml_dis:<17} | {p_ml_one:<10.3e} | {h0_ml_status:<6} | {tk_dis:<17} | {p_tk_one:<10.3e} | {h0_tk_status:<6}")
#                 print("-" * len(header))

# if __name__ == "__main__":
#     run_granular_tables_with_emojis()
